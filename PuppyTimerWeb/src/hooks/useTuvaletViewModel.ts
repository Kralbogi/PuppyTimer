// =============================================================================
// PuppyTimer Web - useTuvaletViewModel Hook
...
export function useTuvaletViewModel(kopekId: number) {
  ...
  const yapayZekaAnaliziBaslat = useCallback(
    async (kayitId: number): Promise<void> => {
      ...
      if (!kayit) {
        setHata('Kayit bulunamadi.');
        return;
      }

      if (!kayit.fotoData) {
        setHata('Analiz icin fotograf gerekli.');
        return;
      }
      ...
      try {
        ...
        setHata(err instanceof Error ? err.message : 'Analiz sirasinda hata olustu.');
      } finally {
