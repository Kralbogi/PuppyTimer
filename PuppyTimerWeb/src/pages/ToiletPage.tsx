// =============================================================================
// PawLand - ToiletPage (Tuvalet Takibi Sayfasi)
...
const tabOptions = [
  { value: "0", label: "Genel Bakış" },
  { value: "1", label: "Büyük Tuvalet" },
  { value: "2", label: "Küçük Tuvalet" },
  { value: "3", label: "Geçmiş" },
];
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {isBuyuk && kayit.sekil && (
                <span
                  ...
                  style={{ background: 'rgba(255,255,255,0.7)', color: 'var(--color-text-muted)' }}
                >
            ...
            {/* Note */}
            {kayit.not && (
              <p className="text-xs mt-2 italic" style={{ color: 'var(--color-text-muted)' }}>{kayit.not}</p>
            )}
          </div>
        </div>
...
const StatCard: React.FC<{
  ...
  <div className={`${color} rounded-xl p-4 flex flex-col items-center`}>
    ...
    <span className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{label}</span>
  </div>
);
...
export const ToiletPage: React.FC<ToiletPageProps> = ({ kopekId }) => {
  ...
  const runAiAnalysis = async () => {
    ...
    try {
      ...
      setAiError(
        err instanceof Error ? err.message : "Analiz sırasında hata oluştu."
      );
    } finally {
      setAiLoading(false);
    }
  ...
  return (
    <div className="px-4 py-4" style={{ background: 'linear-gradient(180deg, transparent 0%, rgba(224, 122, 47, 0.03) 100%)' }}>
      {/* Warning banner */}
      {vm.sonUyarilar.length > 0 && (
        <div className="border rounded-xl px-4 py-3 mb-4 flex items-start gap-3 soft-shadow" style={{ background: 'rgba(239, 68, 68, 0.05)', borderColor: 'rgba(239, 68, 68, 0.3)' }}>
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: 'var(--color-error)' }}>
              Dikkat! Son kayıtlarda uyarı var
            </p>
            <p className="text-xs mt-0.5" style={{ color: 'rgba(239, 68, 68, 0.8)' }}>
              {vm.sonUyarilar.length} kayıtta anormal bulgu tespit edildi.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 mb-4">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 text-white font-semibold py-3.5 rounded-2xl smooth-transition card-hover-lift active:scale-95"
      ...
      {activeTab === "1" && (
        <div>
          {buyukKayitlar.length === 0 ? (
            <EmptyState
              icon={Leaf}
              title="Büyük Tuvalet Kaydı Yok"
              description="Henüz büyük tuvalet kaydı eklenmedi."
            />
          ) : (
      ...
      {activeTab === "2" && (
        <div>
          {kucukKayitlar.length === 0 ? (
            <EmptyState
              icon={Droplets}
              title="Küçük Tuvalet Kaydı Yok"
              description="Henüz küçük tuvalet kaydı eklenmedi."
            />
          ) : (
        ...
        <div>
          {vm.kayitlar.length === 0 ? (
            <EmptyState
              ...
              description="Henüz tuvalet kaydı eklenmedi."
            />
          ) : (
            ...
            <div className="px-5 py-4 space-y-5">
                  ...
                  {diskiRenkListesi.map((r) => (
                    ...
                    >
                      <span
                        ...
                        style={{
                          backgroundColor: diskiRenkRenk(r),
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all resize-none text-sm"
                  style={{
                    border: '1px solid var(--color-border)',
                  ...
                  {idrarRenkListesi.map((r) => (
                    ...
                    >
                      <span
                        className="w-4 h-4 rounded-full flex-shrink-0"
                        style={{
                          backgroundColor: idrarRenkRenk(r),
                          border: '1px solid var(--color-border)',
                        }}
                      />
                      {idrarRenkBaslik(r)}
