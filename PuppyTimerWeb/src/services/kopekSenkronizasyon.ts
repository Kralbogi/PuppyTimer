// =============================================================================
// PuppyTimer Web - Kopek Senkronizasyon Servisi
    ...
    if (snapshot.size > 0) {
      ...
      for (const mevcutId of mevcutIdler) {
        if (mevcutId && !firestoreIdler.has(mevcutId)) {
          ...
          console.log(`🗑️ Yerel kopek silindi (Firestore'da yok): ${mevcutId}`);
        }
      }
      ...
      if (yerelKopekler.length > 0) {
        ...
        console.log(`ℹ️ Hic kopek bulunamadi (ne Firestore'da ne yerel'de)`);
      }
    }
  } catch (error) {
    console.error("❌ Kopekler yuklenemedi:", error);
    // Hata olsa bile yerel verileri KORUMAK icin throw yapma
    console.log("⚠️ Firestore senkronizasyonu basarisiz, yerel veriler korunuyor");
  }
}
...
export async function kopekFirestoreKaydet(kopek: Kopek): Promise<void> {
  try {
    ...
    if (dataSize > 900000) { // 900KB'den büyükse uyar
      console.warn(`⚠️ Veri boyutu çok büyük! (${dataSize} bytes)`);
    }
