// =============================================================================
// PawLand - MapPage (Harita Sayfasi)
...
import KopekAvatarMarker from "../components/common/KopekAvatarMarker";
...
import { useKonum } from "../contexts/KonumContext";
...
// Mode options removed — community toggled via icon button
...
export const MapPage: React.FC<MapPageProps> = ({ kopekId }) => {
  ...
  const [isaretciModu, setIsaretciModu] = useState(false);
  ...
  // TPS / GOD mod — kopek avatari haritada gorunur
  const [gorunumModu, setGorunumModu] = useState<"god" | "tps">("god");
  ...
  // Topluluk hint popup'ı (dismissible)
  const [toplulukHintGorunur, setToplulukHintGorunur] = useState(true);

  // Kullanıcı konumunu takip et (yol tarifi aktifken VEYA TPS modundayken)
  const { konum: kullaniciKonumuTakip, yukleniyor: konumYukleniyor, hata: konumHata } = useKonumTakip(aktifYolTarifi !== null || gorunumModu === "tps");
  ...
  const mapCenter = useMemo<[number, number]>(() => {
    ...
    if (kullaniciBaslangicKonumu) {
      return kullaniciBaslangicKonumu;
    }
    return [varsayilanKonum.lat, varsayilanKonum.lng];
  }, [vm.isaretciler, kullaniciBaslangicKonumu]);
  ...
  const handleMapClick = (lat: number, lng: number) => {
    ...
    } else if (isaretciModu) {
      setAddForm({
        ...initialAddForm,
        baslik: isaretciTuruBaslik(initialAddForm.tur),
        enlem: lat,
        boylam: lng,
      });
      setShowAddModal(true);
      setIsaretciModu(false);
    }
  };
  ...
  const kopeginiPaylas = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof GeolocationPositionError) {
        alert("Konum alınamadı. Lütfen Sistem Ayarları > Gizlilik ve Güvenlik > Konum Servisleri'nden PawLand'e izin verin.");
      } else {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Map Container — full height, no mode selector bar */}
      <div className="relative flex-1">
        ...
        >
          ...
          {/* Kopek avatari — kullanicinin konumunda */}
          {kullaniciBaslangicKonumu && kopek && (
            <KopekAvatarMarker
              konum={kullaniciKonumuTakip || kullaniciKonumu || kullaniciBaslangicKonumu}
              thumbnailData={kopek.fotoData}
              kopekAd={kopek.ad}
              cinsiyet={kopek.cinsiyet}
              tpsMode={gorunumModu === "tps"}
            />
          )}
        ...
        {/* Sol üst: Köpek profil avatarı + TPS/GOD toggle */}
        <div className="absolute top-3 left-14 z-[1000] flex items-center gap-2">
          {/* Kopek avatari — profile link */}
          {kopek && (
            <button
              type="button"
              onClick={() => navigate(`/dog/${kopekId}`)}
              className="w-10 h-10 rounded-full shadow-md border-2 overflow-hidden"
              style={{ borderColor: 'var(--color-primary)', background: 'var(--color-bg-card)' }}
            >
              {kopek.fotoData ? (
                <img
                  src={`data:image/jpeg;base64,${kopek.fotoData}`}
                  alt={kopek.ad}
                  className="w-full h-full object-cover"
                />
              ) : (
                <div className={`w-full h-full flex items-center justify-center text-xs font-bold ${kopek.cinsiyet === "Erkek" ? "bg-blue-100 text-blue-500" : "bg-pink-100 text-pink-500"}`}>
                  {kopek.ad.charAt(0)}
                </div>
              )}
            </button>
          ...
          <button
            type="button"
            onClick={() => setGorunumModu((p) => (p === "god" ? "tps" : "god"))}
            ...
            style={{
              background: gorunumModu === "tps" ? 'var(--color-accent-warm)' : 'var(--color-bg-card)',
              color: gorunumModu === "tps" ? 'white' : 'var(--color-text-muted)',
              border: gorunumModu === "tps" ? 'none' : '1px solid var(--color-border)',
            }}
            title={gorunumModu === "tps" ? "GOD Modu" : "TPS Modu"}
          >
            {gorunumModu === "tps" ? <Locate size={17} /> : <Eye size={17} />}
          </button>
        </div>
        ...
        <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2 items-end">
          {/* Topluluk aç/kapat butonu */}
          <button
            type="button"
            onClick={() => {
              setMapMode((p) => {
                const next = p === "benim" ? "topluluk" : "benim";
                if (next === "benim") setSecilenKopek(null);
                return next;
              });
            }}
            className="w-10 h-10 rounded-full shadow-md flex items-center justify-center transition-colors"
            style={{
              background: isCommunity ? 'var(--color-primary)' : 'var(--color-bg-card)',
              color: isCommunity ? 'white' : 'var(--color-text-muted)',
              border: isCommunity ? 'none' : '1px solid var(--color-border)',
            }}
            title={isCommunity ? "Kendi Haritam" : "Topluluk"}
          >
            <Users size={17} />
          </button>
          ...
          <button
            ...
            className="w-10 h-10 rounded-full shadow-md flex items-center justify-center transition-colors"
            style={{
              background: filtrePaneliAcik ? 'var(--color-text)' : 'var(--color-bg-card)',
              color: filtrePaneliAcik ? 'white' : 'var(--color-text-muted)',
              border: filtrePaneliAcik ? 'none' : '1px solid var(--color-border)',
            }}
          >
            <Filter size={17} />
          </button>

          {/* İşaretçi Ekle butonu (her iki modda da görünür) */}
          <button
            type="button"
            onClick={() => setIsaretciModu(!isaretciModu)}
            className="w-10 h-10 rounded-full shadow-md flex items-center justify-center transition-colors"
            style={{
              background: isaretciModu ? '#c0392b' : 'var(--color-bg-card)',
              color: isaretciModu ? 'white' : 'var(--color-primary)',
              border: isaretciModu ? 'none' : '1px solid var(--color-border)',
            }}
          >
            {isaretciModu ? <X size={17} /> : <Plus size={17} />}
          </button>

          {/* Açılır filtre paneli */}
          {filtrePaneliAcik && (
            <div className="mt-2 backdrop-blur-sm rounded-xl shadow-lg p-3 flex flex-col gap-2 min-w-[180px]" style={{ background: 'rgba(255,252,247,0.95)' }}>
              {!isCommunity && (
                <>
        ...
        {/* İşaretçi modu banner */}
        {!isCommunity && isaretciModu && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[1001] text-white px-4 py-2 rounded-full shadow-lg text-sm font-semibold flex items-center gap-2" style={{ background: 'var(--color-primary)' }}>
            <MapPin size={16} />
            Haritaya dokunarak işaretçi ekleyin
            <button
              type="button"
              onClick={() => setIsaretciModu(false)}
        ...
        {isCommunity && !secilenKopek && (
          benimPaylasilanKopek ? (
            ...
            >
              ...
              {paylasiliyor ? "Durduruluyor..." : "Paylaşımı Durdur"}
            </button>
          ) : (
            <button
              type="button"
              ...
              className="absolute bottom-4 right-4 z-[1000] flex items-center gap-2 px-4 py-3 disabled:opacity-50 text-white font-semibold rounded-full shadow-lg transition-colors"
            ...
            >
              ...
              {paylasiliyor ? "Paylaşılıyor..." : "Köpeğimi Paylaş"}
            </button>
          )
        )}
        ...
        {/* Topluluk hint popup — dismissible */}
        {isCommunity && toplulukHintGorunur && topluluk.bolgeler.length === 0 && !secilenKopek && !secilenBolge && (
          <div className="absolute bottom-28 left-1/2 -translate-x-1/2 z-[1001] backdrop-blur-sm rounded-xl shadow-lg px-4 py-3 flex items-center gap-3 max-w-[300px]" style={{ background: 'rgba(255,252,247,0.95)', border: '1px solid var(--color-border)' }}>
            <MapPin size={18} style={{ color: 'var(--color-primary)' }} className="flex-shrink-0" />
            <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Haritaya dokunarak topluluk bölgesi ekleyebilirsiniz.</p>
            <button
              type="button"
              onClick={() => setToplulukHintGorunur(false)}
      ...
      {/* Bottom panel — personal markers or community interactions */}
      {/* Personal mode: show when markers exist */}
      {!isCommunity && vm.filtrelenmisIsaretciler.length > 0 && (
        <div className="px-4 pt-3 pb-20 border-t" style={{ background: 'var(--color-bg-card)', borderColor: 'var(--color-border)' }}>
      ...
      {/* Community mode: show bottom panel only when there's active content */}
      {isCommunity && (secilenKonusma || secilenBolge || secilenKopek || topluluk.bolgeler.length > 0) && (
        <div className="px-4 pt-3 pb-20 border-t" style={{ background: 'var(--color-bg-card)', borderColor: 'var(--color-border)' }}>
          {secilenKonusma ? (
            <MesajPanel
      ...
      {/* ================================================================== */}
      {showAddModal && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4" style={{ background: 'rgba(61, 46, 31, 0.45)' }}>
          <div className="rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-xl" style={{ background: 'var(--color-bg-card)' }}>
            ...
            <div className="flex gap-3 px-5 py-4 border-t" style={{ borderColor: 'var(--color-border)' }}>
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold transition-colors"
                style={{ background: 'var(--color-primary)' }}
              >
                Kaydet
              </button>
